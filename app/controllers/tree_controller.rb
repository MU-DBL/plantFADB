class TreeController < ApplicationController
  ...
  def data
    ...
    if (!params[:measure_id].blank? || !params[:category].blank? || !params[:cache_json].blank?)
      ...
      if (!params[:cache_json].blank?)
        write_json(tree)
      end
      expires_in 2.hours
      render json: tree
    else
      tree = File.read('public/tree_nodes.json')
      render json: tree
    end
  end

  def write_json(tree)
    File.open("public/tree_nodes.json", "w") do |f|
